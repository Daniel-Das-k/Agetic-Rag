...
input_future = None
unique_id = uuid.uuid4()
CHROMA_DB_PATH = f"/tmp/chromadb-{unique_id}"
...
user_proxy = MyConversableAgent(
    name="Admin",
    is_termination_msg=lambda x: x.get("content", "").rstrip().endswith("exit"),
    system_message="""You are a human admin. Your role is to approve or modify the questions generated by the RAG proxy.""",
    code_execution_config=False,
    human_input_mode="ALWAYS",
    llm_config=llm_config,
)
...
async def callback(contents: str, user: str, instance: pn.chat.ChatInterface):
    ...
    if not initiate_chat_task_created:
        print("Initializing delayed chat...")
        asyncio.create_task(delayed_initiate_chat(user_proxy, manager, contents))
    else:
        if input_future and not input_future.done():
            print("Setting input future result...")
            input_future.set_result(contents)
        else:
            print("No input awaited currently.")
...
# Debugging logs to track issues
