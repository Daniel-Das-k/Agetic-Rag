...
//               </div>
//               <div>
//                 <h1 className="text-xl font-semibold text-gray-900">EduScan</h1>
...
function App() {
  ...
  const [isDragging, setIsDragging] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [analysisData, setAnalysisData] = useState<any | null>(null);  // Store backend data
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const data = response.data;
      setAnalysisData(data);  // Store backend response
      setFile(null);  // Clear the file to hide the PDF preview
      console.log('Success:', data);
    } catch (error) {
      console.error('Error:', error);
    } finally {
